...
# allow environment variable to set the tmp dir location for sandboxed client init phase
if [[ -z "${SANDBOX_TEZOS_TMP_CLIENT}" ]]; then
  TMP_CLIENT="$(mktemp -d -t tezos-tmp-client.XXXXXXXX)"
else
  TMP_CLIENT="$SANDBOX_TEZOS_TMP_CLIENT"
fi
